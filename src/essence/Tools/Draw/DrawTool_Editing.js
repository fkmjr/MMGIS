...
var Editing = {
    ...
    "<div class='drawToolContextMenu'>",
        ...
        $('.drawToolContextMenuReset').on('click', function () {
            ...
            if (DrawTool.plugins?.ScienceTargets?.custom?.updateUi)
                DrawTool.plugins.ScienceTargets.custom.updateUi(
                    DrawTool.contextMenuLayer?.feature?.properties || {}
                )
        ...
        function setProperties(newProperties) {
            ...
            Object.assign(
                newProperties,
                Editing.getPluginSerializedProperties({
                    force: force,
                    properties: properties,
                    newProperties: newProperties,
                })
    ...
    getPluginTabButtons: function (ownedByUser, isMaster, displayOnly, isKind) {
        const tabButtons = []
        Object.keys(DrawTool.plugins).forEach((p) => {
            if (typeof DrawTool.plugins[p].getTabButton === 'function') {
                const tabButton = DrawTool.plugins[p].getTabButton(
                    ownedByUser,
                    isMaster,
                    displayOnly,
                    isKind
                )
                if (tabButton) tabButtons.push(tabButton)
            }
        })
        return tabButtons.join('\n')
    },
    ...
    getPluginSerializedProperties: function (context) {
        const serializedProperties = {}
        Object.keys(DrawTool.plugins).forEach((p) => {
            if (
                typeof DrawTool.plugins[p]?.custom?.serializeProperties ===
                'function'
            ) {
                Object.assign(
                    serializedProperties,
                    DrawTool.plugins[p].custom.serializeProperties(context)
                )
            }
        })
